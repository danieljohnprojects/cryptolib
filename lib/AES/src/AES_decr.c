/**
 * AES Decryption
 * 
 * Each round of decryption in AES consists of an inverse substitution, an  
 * inverse shift, an inverse mix, and an addition of the round key.
 */

#include <AES.h>
#include <stdbool.h>
#include <stdint.h>

static const uint8_t i_sbox[] = {
    };

/**
 * Substitutes the bytes of a word according to the inverse sbox.
 * 
 * That is:
 * [b0,b1,b2,b3] -> [S(b0), S(b1), S(b2), S(b3)]
 * 
 * @param word A word (32 bits long) to be substituted.
 * 
 * @return The substituted word.
 */
static inline uint32_t i_subword(uint32_t word)
{
    uint32_t b3 = i_sbox[(word & 0xff)];
    uint32_t b2 = i_sbox[(word & 0xff00) >> BITS_PER_BYTE];
    uint32_t b1 = i_sbox[(word & 0xff0000) >> 2 * BITS_PER_BYTE];
    uint32_t b0 = i_sbox[(word & 0xff000000) >> 3 * BITS_PER_BYTE];

    return (b0 << 3 * BITS_PER_BYTE) ^ (b1 << 2 * BITS_PER_BYTE) ^ (b2 << BITS_PER_BYTE) ^ b3;
}

/**
 * Substitutes the bytes of a block according to the inverse sbox.
 * 
 * @param in
 * @param out   A buffer to hold the substituted bytes.
 */
static inline void i_subBlock(block_t *in, block_t *out)
{
    for (int i = 0; i < WORDS_PER_BLOCK; i++)
        out->words[i] = i_subword(in->words[i]);

    return;
}

/**
 * Undo the rotation of bytes performed in the encryption phase.
 * 
 * @param in The block to be rotated.
 * 
 * @return The rotated block.
 */
static inline void i_shiftRows(block_t *in, block_t *out)
{
    for (int i = 0; i < BYTES_PER_BLOCK; i++)
    {
        int j = (i - (BYTES_PER_WORD * (i % BYTES_PER_WORD))) % BYTES_PER_BLOCK;
        if (j < 0)
            j += BYTES_PER_BLOCK;
        out->bytes[i] = in->bytes[j];
    }
    return;
}

// To invert the mixColumns step we need to multiply bytes by 9, 11, 13, and 14
// in GF(2^8). Rather than actually computing these we will just use lookup
// tables.
static const uint8_t times9[] = {
 };
static const uint8_t times11[] = {

};
static const uint8_t times13[] = {

};
static const uint8_t times14[] = {

};

/**
 * Inverts the mixcolumns step of AES.
 * 
 * This is equivalent to treating the columns of a block of data as a cubic 
 * over GF(2^8) and multiplying by the fixed polynomial 11z^3 + 13z^2 + 9z + 14 
 * modulo z^4 + 1.
 * 
 * @param in The block that will have its columns (inverse) mixed.
 * @param out A buffer to hold the result of the inverse mixing.
 */
static inline void i_mixColumns(block_t *in, block_t *out)
{
    for (int j = 0; j < 4; j++)
    {
        out->bytes[4 * j] =                 /* b0 = sum of: */
            times14[in->bytes[4 * j]] ^     /* 14*a0 */
            times11[in->bytes[4 * j + 1]] ^ /* 11*a1 */
            times13[in->bytes[4 * j + 2]] ^ /* 13*a2 */
            times9[in->bytes[4 * j + 3]];   /*  9*a3 */
        out->bytes[4 * j + 1] =             /* b1 = sum of: */
            times9[in->bytes[4 * j]] ^      /*  9*a0 */
            times14[in->bytes[4 * j + 1]] ^ /* 14*a1 */
            times11[in->bytes[4 * j + 2]] ^ /* 11*a2 */
            times13[in->bytes[4 * j + 3]];  /* 13*a3 */
        out->bytes[4 * j + 2] =             /* b2 = sum of: */
            times13[in->bytes[4 * j]] ^     /* 13*a0 */
            times9[in->bytes[4 * j + 1]] ^  /*  9*a1 */
            times14[in->bytes[4 * j + 2]] ^ /* 14*a2 */
            times11[in->bytes[4 * j + 3]];  /* 11*a3 */
        out->bytes[4 * j + 3] =             /* b3 = sum of: */
            times11[in->bytes[4 * j]] ^     /* 11*a0 */
            times13[in->bytes[4 * j + 1]] ^ /* 13*a1 */
            times9[in->bytes[4 * j + 2]] ^  /*  9*a2 */
            times14[in->bytes[4 * j + 3]];  /* 14*a3 */
    }
    return;
}

/**
 * Performs a decryption round on the given block.
 * 
 * Decryption round consists of the following steps:
 *  1. Inverse column mixing. (If not final round).
 *  2. Inverse row shifting.
 *  3. Inverse sbox byte substitution.
 */
void decryption_round(block_t *in, block_t *out, bool first)
{
    if (first)
    {
        block_t unshifted;
        i_shiftRows(in, &unshifted);
        i_subBlock(&unshifted, out);
    }
    else
    {
        block_t unmixed;
        i_mixColumns(in, &unmixed);
        block_t unshifted;
        i_shiftRows(&unmixed, &unshifted);
        i_subBlock(&unshifted, out);
    }
    return;
}

/**
 * Compute bytewise xor of two blocks.
 * 
 * @param b1
 * @param b2
 * @param result a buffer to hold the result.
 */
static inline void xor_blocks(block_t *b1, block_t *b2, block_t *result)
{
    for (int i = 0; i < WORDS_PER_BLOCK; i++)
        result->words[i] = b1->words[i] ^ b2->words[i];
    return;
}

/**
 * Decrypt a block of data using AES.
 * 
 */
void decrypt(AES_key *key, block_t *in, block_t *out)
{
    block_t tmp;
    // Addition of final round key
    xor_blocks(&(key->schedule[ROUND_KEYS]), in, out);
    decryption_round(out, &tmp, true);
    for (int round = ROUND_KEYS - 1; round > 0; round--)
    {
        xor_blocks(&tmp, &(key->schedule[round]), out);
        decryption_round(out, &tmp, false);
    }
    xor_blocks(&tmp, &(key->schedule[0]), out);
    return;
}