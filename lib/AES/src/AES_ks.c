...
/**
 * Rotates a 4 byte array to the left by one byte.
 * 
 * For example:
 * [0x01, 0x23, 0x45, 0x67] -> [0x23, 0x45, 0x67, 0x01].
 * 
...
/**
 * Substitute each of the bytes in the given word according to the AES sbox.
 * 
 * Substitution is performed in place.
 * 
 * @param word  The word to be substituted.
 */
static void subword(uint8_t word[BYTES_PER_WORD])
{
    for (int i = 0; i < BYTES_PER_WORD; i++)
        word[i] = sbox[word[i]];
    return;
