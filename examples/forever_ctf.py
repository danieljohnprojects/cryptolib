...
class ForeverCTF_AES_Oracle:
    ...
    def __call__(self, message: bytes) -> bytes:
        ...
        l = self.conn.recvline() # Here's your encrypted message with the flag! Too bad you can't read it now >:D
...
prefix_len, suffix_len = get_additional_message_len(oracle, B, allowable_bytes=allowable_bytes)
print(f"Prefix length: {prefix_len}") # 0
print(f"Suffix length: {suffix_len}") # 41

suffix = decrypt_suffix(oracle, suffix_len=suffix_len, prefix_len=prefix_len, block_size=B, allowable_bytes=allowable_bytes)
