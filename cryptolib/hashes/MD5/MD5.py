...
libpath = build_filename('build/lib/MDHashes/libMD5.so')
MD5libC = CDLL(libpath)
init_buffer = bytes.fromhex("0123456789abcdeffedcba9876543210")


def md5digest(message: bytes) -> bytes:
    """
    ...
    MD5libC.md5digest(message, len(message), 0, digest_buffer)
    return digest_buffer.raw

def md5extend(prev_hash: bytes, prev_len: int, message: bytes) -> bytes:
    """
    Extend the given MD5 hash to a hash of the original message plus padding concatenated with a new message.

    Args:
        prev_hash: The MD5 hash of a message. Nothing about the original message itself need be known except for its length.
    ...
    MD5libC.md5digest(message, len(message), prev_len, digest_buffer)
    return digest_buffer.raw

def md5extend_message(prefix_len: int, message: bytes, suffix: bytes) -> bytes:
    """
    ...
    Returns:
        A message that should pass a prefix MAC verification oracle (the corresponding MAC can be calculated with the md5extend function). 
    """
