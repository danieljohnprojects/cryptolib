...
DH_Public_Key = Tuple[int, int, int]  # Modulus p, base point g, g**x
...
def DH_Factory(
...
) -> Tuple[Callable[[], Tuple[DH_Private_Key, DH_Public_Key]], Callable[[DH_Private_Key, DH_Public_Key], bytes]]:
    """
    ...
    """
    def key_generation_oracle() -> Tuple[DH_Private_Key, DH_Public_Key]:
    ...
    def key_agreement_oracle(private: DH_Private_Key, pub: DH_Public_Key) -> bytes:
        ...
        if p1 != p:
            raise RuntimeError(
                f"Incompatible public modulus! My modulus is {p}, I was asked to form a shared secret with a modulus of {p1}.")
        if g1 != g:
            raise RuntimeError(
                f"Incompatible public base point! My base point is {g}, I was asked to form a shared secret with a base point of {g1}.")
        shared_secret = pow(pk, private, p)
